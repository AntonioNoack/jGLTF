...
vec4 computeEmissive(){
    if (u_hasEmissiveTexture != 0){
        vec4 sampledEmissive = texture2D(u_emissiveTexture, v_emissiveTexCoord);
        return sampledEmissive * vec4(u_emissiveFactor, 1.0);
    } else {
        return vec4(u_emissiveFactor, 1.0);
    }
}
...
void main(){
    ...
    // todo probably this will be incorrect once we use it, and should be 0,0,0
    ...
    vec4 combinedColor = clamp(mainColor * occlusionFactor + emissive, 0.0, 1.0);

    // todo define a deferred renderer and a direct renderer with these variables
    // todo then write here the correct, raw color
    vec3 finalColor = combinedColor.rgb;
    float finalAlpha = combinedColor.a;

    vec3 finalPosition = v_position;
    vec4 finalEmissive = emissive;
    // todo encode high exponents for the emissive
